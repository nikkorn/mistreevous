...
describe("A While guard node attribute", () => {
    ...
    describe("when the node is updated as part of a tree step will call the guard function and", () => {
        describe("abort the running node and any child nodes and if the function returns a value of false will move them to the", () => {
            describe("FAILED state if", () => {
                ...
                describe("the guard is configured to move the aborted node to the FAILED state", () => {
                    it("(MDSL)", () => {
                        const definition = `root { sequence while(someCondition, "condition-argument") then fail { action [someAction] exit(onActionExit) } }`;
                        const agent = {
                            someAction: () => State.RUNNING,
                            someCondition: sinon.stub().returns(true),
                            onActionExit: sinon.stub()
            ...
            describe("SUCCEEDED state if the guard is configured to move the aborted node to the SUCCEEDED state", () => {
                it("(MDSL)", () => {
                    const definition = `root { sequence while(someCondition, "condition-argument") then succeed { action [someAction] exit(onActionExit) } }`;
                    const agent = {
                        someAction: () => State.RUNNING,
                        someCondition: sinon.stub().returns(true),
                        onActionExit: sinon.stub()
